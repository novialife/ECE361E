...
class SimpleFC(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.dropout(out) # unsure if this is correct
...
for epoch in range(num_epochs):
    ...
    total_train_time += starter.elapsed_time(end)
...
print('Total training time: %.2f seconds' % (total_train_time / 1000))

# Plot the training and testing losses and accuracies for each epoch on separate plots
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Training and testing losses for dropout probability ' + str(prob))
plt.savefig('losses_fc_' + str(prob) + '.png')
